-- ============================================================================
-- MIGRATION 032: AI ANALYSIS ENHANCED METRICS
-- 
-- PURPOSE: ADD-ON refinement to get_ai_analysis_data RPC function
-- Adds missing fields to team_totals and new analysis objects
-- 
-- CHANGES:
-- 1. team_totals: Add assists, fouls, points, fg_*, three_* (match opponent_totals)
-- 2. NEW: shooting_comparison - 3PT differential, true FG%
-- 3. NEW: efficiency_metrics - A:TO ratio, steal-turnover correlation
-- 4. NEW: bench_players - Players ranked 5+ by impact score
--
-- NOTE: This is an ADD-ON migration - existing fields are NOT changed
-- ============================================================================
...
BEGIN
  ...
  -- =========================================================================
  -- STEP 4: PLAYER STATS WITH IMPACT SCORES (UNCHANGED)
  -- =========================================================================
  player_stats_raw AS (
    SELECT 
    ...
    SELECT 
      *,
      ...
      ROW_NUMBER() OVER (ORDER BY 
        ROUND(
          (SUM(CASE WHEN stat_type IN ('field_goal', 'two_pointer') AND modifier = 'made' THEN 2
                   WHEN stat_type = 'three_pointer' AND modifier = 'made' THEN 3
                   WHEN stat_type = 'free_throw' AND modifier = 'made' THEN 1
                   ELSE 0 END) OVER (PARTITION BY COALESCE(player_id::text, custom_player_id::text))) * 1.0 + 
          (rebounds * 0.8) + 
          (assists * 0.7) + 
          (steals * 1.2) + 
          (blocks * 1.1) - 
  ...
  SELECT jsonb_build_object(
    ...
    -- =====================================================================
    -- TEAM_TOTALS - ENHANCED (Added missing fields to match opponent_totals)
    -- =====================================================================
    'team_totals',
    (SELECT jsonb_build_object(
      ...
      'ft_percentage', CASE 
        ...
        ELSE 0
      END,
      -- NEW fields (added to match opponent_totals)
      ...
      'fg_made', COALESCE(SUM(two_pt_made), 0),
      'fg_attempted', COALESCE(SUM(two_pt_attempted), 0),
      'fg_percentage', CASE 
    ...
    -- =====================================================================
    -- OPPONENT_TOTALS - ENHANCED (Added offensive/defensive rebounds)
    -- =====================================================================
    'opponent_totals',
    (SELECT jsonb_build_object(
    ...
    -- =====================================================================
    -- NEW: BENCH PLAYERS (Players ranked 5+ by impact score)
    -- =====================================================================
    'bench_players',
    ...
    -- =====================================================================
    -- NEW: SHOOTING COMPARISON (3PT differential, true FG%)
    -- =====================================================================
    'shooting_comparison',
    (SELECT jsonb_build_object(
    ...
    -- =====================================================================
    -- NEW: EFFICIENCY METRICS (A:TO ratio, steal-turnover correlation)
    -- =====================================================================
    'efficiency_metrics',
    (SELECT jsonb_build_object(
      ...
      -- Steal-Turnover Correlation (opponent steals vs your turnovers)
...
-- ============================================================================
-- VERIFICATION: Test the enhanced RPC
-- ============================================================================
-- Run this after applying migration to verify new fields:
-- SELECT jsonb_object_keys(get_ai_analysis_data('b7f9757a-4205-4784-ade4-296e2817d55a'::uuid));
-- 
-- Expected output should now include:
-- game, players, quarters, team_totals, opponent_totals, bench_players, shooting_comparison, efficiency_metrics
