-- =========================================================================
-- Migration 033: Fix AI Analysis RPC Window Function Bug
-- =========================================================================
-- 
-- ISSUE: The player_stats_with_impact CTE contains a window function that
--        references columns (stat_type, modifier) that don't exist in its
--        source (player_stats_raw) because they were already aggregated away.
--
-- ERROR: "column stat_type does not exist"
--
-- FIX: Use the already-computed columns (points, rebounds, etc.) instead of
--      trying to re-aggregate from raw columns that no longer exist.
--
...
BEGIN
  ...
  -- =========================================================================
  -- STEP 4: PLAYER STATS WITH IMPACT SCORES (FIXED - window function bug)
  -- =========================================================================
  player_stats_raw AS (
    SELECT 
  ...
  player_stats_with_impact AS (
    SELECT 
      *,
      ...
      -- ✅ FIXED: Use already-computed columns instead of non-existent stat_type/modifier
