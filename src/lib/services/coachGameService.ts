...
export class CoachGameService {
  ...
  static async createQuickTrackGame(request: QuickTrackGameRequest): Promise<CoachGame> {
    try {
      ...
      // Create the game with coach-specific fields
      const { data, error } = await supabase
        ...
        .insert({
          ...
          stat_admin_id: user.id, // Coach is the stat admin for their own games
          
          // Team assignment (coach team as team_a, opponent as team_b placeholder)
          team_a_id: request.coach_team_id,
          team_b_id: request.coach_team_id, // Placeholder, not used in coach mode
          ...
          meta_json: {
            ...
            venue: request.game_settings?.venue,
            notes: request.game_settings?.notes,
            shot_clock_seconds: request.game_settings?.shot_clock_seconds || 24
          }
        })
        .select()
        .single();
