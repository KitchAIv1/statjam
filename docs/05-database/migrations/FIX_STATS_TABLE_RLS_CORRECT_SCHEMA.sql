-- ============================================================================
-- FIX: RLS Policy for 'stats' table (CORRECT SCHEMA)
-- 
...
-- - Uses 'match_id' (not 'game_id') as foreign key to games(id)
-- - Uses 'player_id' as foreign key to users(id)
-- - Aggregated statistics with JSONB fields
-- ============================================================================
...
DROP POLICY IF EXISTS "stats_player_read_self" ON public.stats;

-- Step 5: Create comprehensive RLS policies for stats table (CORRECT SCHEMA)

-- Policy 1: Stat admins can manage (SELECT, INSERT, UPDATE, DELETE) stats for their assigned games
-- USES CORRECT COLUMN: match_id (not game_id)
CREATE POLICY "stats_stat_admin_manage" ON public.stats
FOR ALL TO authenticated
USING (
  EXISTS (
    SELECT 1 FROM public.games g
    WHERE g.id = stats.match_id AND g.stat_admin_id = auth.uid()
  )
)
WITH CHECK (
  EXISTS (
...
SELECT '=== STATS TABLE RLS SETUP COMPLETE (CORRECT SCHEMA) ===' as status;
