...
**Recent Achievements** ✅:
1. **🏀 v0.17.14 – PUBLIC STANDINGS DIFF ADJUSTMENTS (Mar 2026)**: Optional per-team point differential deltas for public tournament standings via `tournaments.ruleset_config.standingsPointDifferentialAdjustments` (display + sort only; PF/PA/W-L unchanged). Implemented in `TournamentStandingsService`; documented in [STANDINGS_POINT_DIFFERENTIAL_ADJUSTMENTS.md](../02-development/STANDINGS_POINT_DIFFERENTIAL_ADJUSTMENTS.md). `package.json` and docs hub version aligned to **0.17.14**.
2. **📊 v0.17.12 – ANALYTICS, SEARCH & HARDENING (Feb 2026)**: Google Analytics 4 and event tracking (player/tournament/game views, sign up, tournament/team created, live stream, video upload). Global search (useGlobalSearch + GlobalSearchBar). Sentry for video upload flow and useGlobalSearch. CSP updated for GA4; www→apex redirect and canonical/OG URLs. Overlay: schedule/lineup on broadcast canvas, contrast-safe bars, free throw avatar, image load timeout. Tracker: clock device-switch fix, quarter length source of truth, foul dots UI, pre-flight auto/advanced. Relay: EBML-gated reconnection, AU region selector, lower default bitrate. useGameReplays score fallback and stat_value alignment. Full audit: [COMMITS_AUDIT_SINCE_FEB_2026.md](COMMITS_AUDIT_SINCE_FEB_2026.md).
3. **🏀 TOURNAMENT PUBLIC PAGE & MONITORING (Feb 2026)**: Tournament public page hydration fix (client-only `TournamentTabsSection`), rectangular player/team cards with full-height photos/logos, schedule tab infinite-loop fix (stable `roundIndicesKey`), theme class for schedule round header. Sentry error logging across stat tracker (useTracker, useGameDataLoader, turn-credentials API) for faster diagnosis in production. Dependency security: npm audit fix for qs (DoS) and webpack (SSRF); **0 vulnerabilities**.
4. **🎬 VIDEO UPLOAD RELIABILITY & STATUS FIXES (Jan 4, 2025)**: Implemented Bunny.net webhook fallback system for large file uploads (13GB+). Server-side webhook endpoint ensures database records are created even if client callback fails. Fixed video status synchronization - `/api/video/check-status` now automatically updates database when Bunny reports ready. Videos now appear in Admin pipeline immediately after processing. Handles browser closure, session expiration, and network interruptions gracefully. All changes code-only, no database migrations required.
